...
void GCMC::init()
{
    ...
    for (int i = 0;i < ngasType;i++)
    {
        ...
        if(atoms->comm_rank==0)
        {
            ...
            fprintf(fp_debug,"gas mas masses\t%e\n",gas_massArr[i]);
    ...
    MPI_Allreduce(&ngas_lclArr,&ngasArr,1,MPI_INT,MPI_SUM,world);//-mingfei
#ifdef GCMC_2DEBUG
        FILE* fp_debug=NULL;
        if(atoms->comm_rank==0)
        {
            fp_debug=fopen("gcmc_debug","a");
            fprintf(fp_debug,"ngasArr\t%e\t%e\n",ngasArr[0],ngasArr[1]);
        }
#endif
