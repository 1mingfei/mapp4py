...
/*--------------------------------------------
 constructor
 --------------------------------------------*/
PGCMC_2::PGCMC_2(AtomsMD*& __atoms, ForceFieldMD*&__ff,\
        DynamicMD*& __dynamic, int __m, elem_type __gas_type0, type0 __mu0,\
        type0 __T, int seed, elem_type __gas_type1,type0 __mu1, type0 __ratio):
GCMC(__atoms,__ff,__dynamic,__gas_type0,__mu0,__gas_type1,__mu1,__T,seed),
m(__m),
ratio(__ratio)
{
    n_comm=0;
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
void PGCMC_2::xchng(bool chng_box,int nattmpts)
{
    dynamic->init_xchng();
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
inline void PGCMC_2::find_cell_no(type0*& s,int& cell_no)
{
    cell_no=0;
    for(int i=0;i<__dim__;i++)
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
inline void PGCMC_2::find_cell_coord(type0*& s,int*& cell_coord)
{
    for(int i=0;i<__dim__;i++)
    {
        if(s[i]<s_lo[i])
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
inline void PGCMC_2::next_jatm_reg()
{
    while(true)
    {
        ...
        if(jatm==-1)
        {
            ...
            {
                iself=0;
                next_jatm_p=&PGCMC_2::next_jatm_self;
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
inline void PGCMC_2::next_jatm_self()
{
    while(true)
    {
        iself++;
...
void PGCMC_2::del_succ()
{
    ...
    //- mingfei : there is something wrong here. got to check it out
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
void PGCMC_2::comms_setup(int n_vars_,int n_s_)
{
    n_vars=n_vars_;
    n_s=n_s_;
    ip=atoms->comm_rank;
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
void PGCMC_2::create_comm_pattern()
{
    int n,d,r,l;
    n_curr_comms=1;
    im_root=true;
